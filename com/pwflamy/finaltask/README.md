...
1.  **Обработка задач в многопоточном режиме:**
    ...
    *   **Запрещено использовать** `ExecutorService` или другие высокоуровневые механизмы из пакета `java.util.concurrent` для управления пулом потоков. Требуется реализовать свою версию потоков-воркеров и компонента-оркестратора, управляющего их жизненным циклом и координацией.
